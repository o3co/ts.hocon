...
- **プロトタイプ汚染:** `__proto__` / `constructor` / `prototype` という名前のキーは
  **通常のキーとして保持されます**。`.properties` ファイルや環境変数がそれらを正当に
  使っていることがあり、落とせば黙ったデータ欠落になるからです。安全性は拒否リストでは
  なく構造で担保しています。ネスト構築には `__proto__` セッターを継承しない
  null プロトタイプのオブジェクトを使い、`toObject()` は各キーを own データプロパティと
  して定義します。したがって設定由来の `__proto__` はただのキーとして現れ、返される
  オブジェクトのプロトタイプは常に `Object.prototype` で、グローバルな
  `Object.prototype` には何も届きません。なおキー自体は結果に存在するため、設定
  オブジェクトを走査するコードはこれらの名前が無い前提を置かず、`Object.entries()` /
  `Object.keys()` を使ってください。
